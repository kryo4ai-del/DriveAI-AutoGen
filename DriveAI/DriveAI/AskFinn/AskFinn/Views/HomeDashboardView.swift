...
struct HomeDashboardView: View {
    ...
    private func primaryButton(_ label: String, icon: String, color: Color, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Label(label, systemImage: icon)
                ...
                .cornerRadius(AppDesignConstants.cornerRadius)
                .shadow(color: color.opacity(AppDesignConstants.glowOpacity), radius: AppDesignConstants.glowRadius)
                .overlay(
                    RoundedRectangle(cornerRadius: AppDesignConstants.cornerRadius)
                        .stroke(color.opacity(AppDesignConstants.borderOpacity), lineWidth: 1)
                )
        }
    }
    ...
    private func secondaryButton(_ label: String, icon: String, color: Color, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ...
            .cornerRadius(AppDesignConstants.cardCornerRadius)
            .overlay(
                RoundedRectangle(cornerRadius: AppDesignConstants.cardCornerRadius)
