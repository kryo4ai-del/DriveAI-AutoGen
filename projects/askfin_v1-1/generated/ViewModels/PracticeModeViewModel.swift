...
enum PracticeModeNavigation {
    ...
    case showSessionEnd(SessionResult)
}
...
class PracticeModeViewModel: ObservableObject {
    ...
    init(dataService: LocalDataService = DefaultLocalDataService.shared) {
    ...
    func resumeSession(_ sessionId: UUID) async {
        ...
        do {
            guard let session = try await dataService.loadSession(sessionId) else {
                throw DataServiceError.categoryNotFound(sessionId.uuidString)
            }
            ...
            guard session.currentQuestionIndex < questions.count else {
                throw DataServiceError.questionNotFound("index-\(session.currentQuestionIndex)")
            }
    ...
    func selectAnswer(_ index: Int) async {
        ...
        session.answers[question.id] = PracticeSession.SelectedAnswer(
    ...
    private func loadCurrentQuestion(_ question: Question) async {
