...
struct TrainingResult: Identifiable, Codable, Equatable {
    let id: UUID
    let sessionId: UUID
    let categoryName: String
...
// MARK: - TrainingSession Convenience Init

extension TrainingSession {
    /// Convenience init for creating completed sessions from TrainingSessionViewModel.
    init(
        ...
        self.id = UUID()
        self.categoryId = ""
        self.categoryName = String(describing: sessionType)
        self.startedAt = startedAt
        self.completedQuestions = results.map { result in
            CompletedQuestion(
                questionId: result.questionID.uuidString,
                selectedAnswer: result.selectedDirection.answerLetter,
                isCorrect: result.wasCorrect,
                timeSpent: result.answeredAt.timeIntervalSince(startedAt),
                explanationRead: false,
                completedAt: result.answeredAt
            )
        }
        self.currentQuestionIndex = results.count
