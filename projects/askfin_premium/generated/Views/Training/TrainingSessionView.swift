        ...
        case .reveal(let wasCorrect, let missDistance):
            ...
            if let question = viewModel.revealedQuestion {
                AnswerRevealView(
                    ...
                    // Issue 3 fix: lookup moved to ViewModel method, not inline.
    ...
    private var summaryPhase: some View {
        if let session = viewModel.completedSession {
            SessionSummaryView(
                ...
                onDismiss: { dismiss() },
                onTrainWeaknesses: {
                    // MVP: dismiss and return to home.
                    // Future: this closure should receive a NavigationPath
                    // binding or coordinator reference so it can push a new
                    // TrainingSessionView(.weakFocus) without re-entering the
                    // current stack frame.
                    dismiss()
                }
            )
