...
enum AppRoute: Hashable {
    ...
    func hash(into hasher: inout Hasher) {
        ...
        case .quiz(let categoryId): hasher.combine(2); hasher.combine(categoryId)
        case .category(let id): hasher.combine(3); hasher.combine(id)
        case .exam: hasher.combine(4)
        case .results(let resultId): hasher.combine(5); hasher.combine(resultId)
        case .profile: hasher.combine(6)
        }
    }
