    ...
    func loadJSON<T: Decodable>(
        ...
        if let data = userDefaults.data(forKey: key) {
            do {
                ...
                print("[DataPersistenceManager] ERROR: Primary data corrupted for key '\(key)': \(error)")
            }
        }
        ...
        if fallbackToBackup {
            if let backupData = userDefaults.data(forKey: key + ".backup") {
                do {
                    ...
                    print("[DataPersistenceManager] WARNING: Recovered from backup for key '\(key)'")
                    userDefaults.set(backupData, forKey: key)
                    return .success(decoded)
                } catch {
                    print("[DataPersistenceManager] ERROR: Backup also corrupted for key '\(key)': \(error)")
                }
            }
        }
