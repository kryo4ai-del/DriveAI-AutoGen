...
# ── Service Provider Integration (Phase 7) ────────────────────────
...
def get_service_registry():
    """Get the Service Registry instance. Lazy-loaded singleton."""
    ...
    if _service_registry is None:
        try:
            ...
            import logging
            logging.getLogger(__name__).error("Failed to load ServiceRegistry: %s", e)
            return None
    return _service_registry


def get_service_router():
    """Get the Service Router instance. Lazy-loaded singleton."""
    ...
    if _service_router is None:
        try:
            ...
            if reg:
                _service_router = ServiceRouter(reg)
        except Exception as e:
            import logging
            logging.getLogger(__name__).error("Failed to load ServiceRouter: %s", e)
            return None
    return _service_router


def get_quality_scorer():
    """Get the Quality Scorer instance. Lazy-loaded singleton."""
    ...
    if _quality_scorer is None:
        try:
            ...
            import logging
            logging.getLogger(__name__).error("Failed to load QualityScorer: %s", e)
            return None
    return _quality_scorer


def get_cost_tracker():
    """Get the Service Cost Tracker instance. Lazy-loaded singleton."""
    global _cost_tracker
    if _cost_tracker is None:
        try:
            from factory.brain.service_provider.cost_tracker import ServiceCostTracker
            _cost_tracker = ServiceCostTracker()
        except Exception as e:
            import logging
            logging.getLogger(__name__).error("Failed to load ServiceCostTracker: %s", e)
            return None
    return _cost_tracker


def get_service_scout():
    """Get the Service Scout instance. Lazy-loaded singleton."""
    global _service_scout
    if _service_scout is None:
        try:
            from factory.brain.service_provider.service_scout import ServiceScout
            reg = get_service_registry()
            router = get_service_router()
            if reg:
                _service_scout = ServiceScout(reg, router)
        except Exception as e:
            import logging
            logging.getLogger(__name__).error("Failed to load ServiceScout: %s", e)
            return None
    return _service_scout
