"""Document Secretary — transforms raw agent reports into professional .docx documents.
...
from factory.document_secretary.docx_builder import DocxBuilder
...
def generate_ceo_briefing_p1(phase1_run_dir: str, send_email: bool = False) -> str:
    """Generate CEO Briefing for Phase 1. Returns path to generated .docx."""
    print(f"[DocumentSecretary] Generating CEO Briefing Phase 1...")
    ...
    filename = f"CEO_Briefing_Phase1_{title.replace(' ', '_')}_{date.today().isoformat()}.docx"
    ...
    if send_email:
        send_document(
            filepath,
            subject=f"CEO Briefing Phase 1: {title}",
            body=f"Anbei das CEO Briefing fuer {title} (Phase 1 — Pre-Production Analysis).",
        )
...
def generate_ceo_briefing_p2(phase2_run_dir: str, phase1_run_dir: str = None, send_email: bool = False) -> str:
    """Generate CEO Briefing for Phase 2. Returns path to generated .docx."""
    print(f"[DocumentSecretary] Generating CEO Briefing Phase 2...")
    ...
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    filename = f"CEO_Briefing_Phase2_{title.replace(' ', '_')}_{date.today().isoformat()}.docx"
    filepath = str(OUTPUT_DIR / filename)
    builder.save(filepath)
    ...
    if send_email:
        send_document(
            filepath,
            subject=f"CEO Briefing Phase 2: {title}",
            body=f"Anbei das CEO Briefing fuer {title} (Phase 2 — Strategie & Positionierung).",
        )
...
def generate_marketing_konzept(phase1_run_dir: str, phase2_run_dir: str, send_email: bool = False) -> str:
    """Generate standalone Marketing Konzept document. Returns path to generated .docx."""
    print(f"[DocumentSecretary] Generating Marketing Konzept...")
    title = _extract_title(phase1_run_dir)
    ...
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    filename = f"Marketing_Konzept_{title.replace(' ', '_')}_{date.today().isoformat()}.docx"
    filepath = str(OUTPUT_DIR / filename)
    builder.save(filepath)
    ...
    if send_email:
        send_document(
            filepath,
            subject=f"Marketing-Konzept: {title}",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DriveAI Document Secretary — Professional .docx Generator")
